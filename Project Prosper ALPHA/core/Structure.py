class Structure:
    def __init__(self,type,id,escape=None,safeZone=None):
        ...
        if self.type=='cave':
            ...
            self.color=PurePath('biomes','cave.png')
        elif self.type=='dTemple':
            self.color=PurePath('biomes','dTemple.png')
        #elif self.biome=='f':
    ...
    def loadTile(self):
        screen.blit(pygame.image.load(self.color),(0,0))
        for obstaclee in self.obstacles:
            if obstaclee.type != 'none':
                screen.blit(pygame.image.load(obstaclee.sprite),obstaclee.rect) 
